...
class Sample(object):
    ...
    def __init__(self, type, arg):
        ...
        if type == 'file':
            ...
            #print ("yama_files is: %s" % files)
            self.job_link = web_link + "/".join(files[0].split("/")[4:6]) + "/results.html"
            #self.job_link = web_link + "/".join(files[0].split("/")[4:7]) + "/job_report.html"
            print files[0]
            configure = files[0].split("/")[7].split(".")
            print ("yama_configure is: %s" % configure)
            print ("aa=%s" % configure[3:9])
            print ("aa=%s" % configure[10])
            print ("aa=%s" % configure[11:14])
            self.config = ".".join(configure[3:9]) + "." + configure[10] + "." + "Guest_" + ".".join(configure[11:14])
        elif type == 'database':
            jobid = arg
        ...
        self.version = " host kernel: %s\n" % self.hostkernel
        nrepeat = len(self.files_dict)
        if nrepeat < 2:
            print "`nrepeat' should be larger than 1!"
            sys.exit(1)

        self.desc = """Every Avg line represents the average value based on <font color=red> *%d* </font> repetitions of the same test,
   and the following SD line represents the Standard Deviation between the <font color=red> *%d* </font> repetitions.
...
""" % (nrepeat, nrepeat)
    ...
    def _get_augment_rate(self, data):
        ...
        result = "+0.0"
        if len(data) == 2 and float(data[0]) != 0:
            result = "%+.4f%%" % ((float(data[1]) - float(data[0]))
                                  / float(data[0]) * 100)
        return result
...
def display(lists, rates, allpvalues, f, ignore_col,
            important_level, increase_good, p_value, sum="Augment Rate",
    ...
    def tee_mark(prefix, value1, value2, file):
        ...
        for i in range(len(pvalue)):
            if ((float(pvalue[i][1:])) > (1 - float(p_value))):
    ...
    for i in range(len(lists[0])):
        ...
        for n in range(len(rates)):
            if lists[0][i] != rates[n][i] and (not re.findall("[a-zA-Z]", rates[n][i]) or "nan" in rates[n][i]):
                if len(rates) > 1:
                    if n == 0:
...
def analyze(test, type, arg1, arg2, configfile):
    ...
    print("yama_avg_update: %s" % avg_update)
    ...
    #s1.len = "<font color=red>" + "*" + s1.len.lstrip() + "*" + "</font>"
    #desc = desc % s1.len

    # highlight the version title
    ver1 = version_mark(s1, s2, arg1)
    tee("<pre>####1. Description of setup#1 --->" + s1.config + " (<a href=%s>" % s1.job_link + arg1.split("/")[5] + "</a>)\n" + ver1 + "</pre>", test + ".html")
    ver2 = version_mark(s2, s1, arg2)
    tee("<pre>####2. Description of setup#2 --->" + s2.config + " (<a href=%s>" % s2.job_link + arg2.split("/")[5] + "</a>)\n" + ver2 + "</pre>", test + ".html")

    tee("<pre>" + '\n'.join(desc.split('\\n')) + "</pre>", test + ".html")
    tee("<pre>" + s1.desc + "</pre>", test + ".html")

    display([avg1, sd1, avg2, sd2], rlist, allpvalues, test + ".html",
            ignore_col, important_level, increase_good, p_value,
            sum="Regression Testing: %s" % test, prefix0="#|Title|",
    ...
    display(s1.files_dict, [avg1], [], test + ".avg.html", ignore_col,
            important_level, increase_good, p_value,
            sum="Raw data of sample 1", prefix0="#|Title|",
            prefix1=[" |    |"],
            prefix2=["-|Avg |"], prefix3="")

    display(s2.files_dict, [avg2], [], test + ".avg.html", ignore_col,
            important_level, increase_good, p_value,
            sum="Raw data of sample 2", prefix0="#|Title|",
            prefix1=[" |    |"],
            prefix2=["-|Avg |"], prefix3="")
...
if __name__ == "__main__":
    ...
    print(sys.argv[3])	
    print(sys.argv[4])	
    analyze(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], 'perf.conf.libfuse')
